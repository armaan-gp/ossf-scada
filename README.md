...
- Running alert processing and dispatching alert emails for new alert episodes (subject to alert cooldown).
...
- Alert events/emails are generated by the cron endpoint (`/api/alerts/run`) and are not triggered by page views.
- Cooldown is enforced per `(thingId, propertyId)`. New out-of-range episodes within cooldown do not generate events/emails.
- After cooldown suppression, a continuously out-of-range property will not auto-retrigger until it first returns in-range and then breaches again.
...
Page purpose:
- Central admin control panel for email alerts, alert cooldown, thresholds, display formatting, and CSV recording config.
...
Validation:
- Hours must be a whole number >= 0.
- Minutes must be a whole number between 0 and 59.
- Total cooldown must be at least 15 minutes.
